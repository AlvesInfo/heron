...
class SalesExcelExporter:
    ...
    SHEET_NAME = "VENTES HERON"
    TITLE = "VENTES HERON A FINALISER"
    START_ROW = 4
    SHEET_NUMBER = 1
    ...
    def __init__(self, file_io: io.BytesIO, file_name: str):
        ...
        self.columns = columns_sales_heron
    ...
    def _fetch_rows() -> Iterator[Tuple]:
        ...
        sql_file_path = (
            Path(settings.APPS_DIR) / "validation_sales/sql_files/sales_not_final.sql"
        )
...
def excel_heron_sales_not_final(file_io: io.BytesIO, file_name: str) -> Dict[str, str]:
    """
    ...
    Returns:
        Dict avec clé 'OK' ou 'KO' selon le résultat
    """
    exporter = SalesExcelExporter(file_io, file_name)
