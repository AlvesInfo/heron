...
def integration_sales(request):
    """View de l'étape 2 des écrans de contrôles
    Visualisation pour la validation des montants de ventes par mois, intégrés ou saisis
    ...
    sql_context_file = "apps/validation_sales/sql_files/sql_integration_sales.sql"

    with connection.cursor() as cursor:
        elements = query_file_dict_cursor(cursor, file_path=sql_context_file)
        context = {
            "titre_table": "2.1 - Contrôle des Intégrations - Ventes",
    ...
    return render(request, "validation_sales/integration_sales.html", context=context)
