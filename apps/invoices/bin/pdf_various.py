...
"""
FR : Module de génération des factures de Diverses en pdf
EN : Module for generating invoices Various in pdf
...
from apps.invoices.sql_files.sql_various import SQL_HEADER, SQL_RESUME_HEADER


def invoice_various_pdf(uuid_invoice: UUID, pdf_path: Path) -> None:
    """
    ...
    with connection.cursor() as cursor:
        ...
        content = render_to_string("invoices/pdf_various.html", context)
...
if __name__ == "__main__":
    uuid_invoice_to_pdf = UUID("5094fec5-06d7-40eb-9b50-aaf2daa272ff")
    sale = SaleInvoice.objects.get(uuid_identification=uuid_invoice_to_pdf)

    various_path = Path(settings.SALES_INVOICES_FILES_DIR) / f"{sale.cct}_various.pdf"
    invoice_various_pdf(uuid_invoice=uuid_invoice_to_pdf, pdf_path=various_path)
