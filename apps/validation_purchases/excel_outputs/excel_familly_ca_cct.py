...
def excel_familly_ca_cct(file_io: io.BytesIO, file_name: AnyStr, client: AnyStr) -> Dict:
    """Fonction de 5.3A - Contrôle CA Cosium / Ventes Héron par familles"""
    titre = "5.3A - Contrôle CA Cosium / Ventes Héron par familles"
    list_excel = [file_io, ["CA CCT PAR FAMILLES"]]
    excel = GenericExcel(list_excel, in_memory=True)
    file_path = Path(f"{str(APPS_DIR)}/validation_purchases/sql_files/sql_ca_familly_cct.sql")
    mois = 1
    ...
    try:
        ...
        row += 1
        rows_writer(
            excel, 1, row, 0, get_rows(file_path, {"client": client}), f_lignes, f_lignes_odd
        )
        sheet_formatting(
